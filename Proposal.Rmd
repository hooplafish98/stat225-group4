...
nbaper36select = select(nbaper36, Player, MP, TRB, BLK, TOV, FTA, PTS, 'FT%') %>% rename(FT = 'FT%')
...
results <- boot(data = nbaper36select, statistic=bs,
   R=1000, formula = MP ~ TRB + BLK + TOV + FTA + PTS + FT)
...
boot.ci(results, type="bca", index=7) # FT%
```
