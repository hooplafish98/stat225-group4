...
summary(ols_mod1)$coefficients
``` 

MP = 9.573 - 1.2937 AST + 2.3198 TOV
0.8941 PTS
...
summary(model_f1)$coefficients
```

MP = 9.294 - 1.2996 AST + 1.2995 TOV
0.9083 PTS 
...
ks.test(x = resid(ols_mod1), y = pnorm, mean = 0, sd = 5.96654)

#JHM
#sd(resid(model_f))

ks.test(x = resid(model_f1), y = pnorm, mean = 0, sd = 5.968016)
```
...
ks.test(x = resid(full_gam1), y = pnorm, mean = 0, sd = 5.670123)
```
...
fit_final = rbind(fit_ols(ols_mod),fit_jhm(model_f),fit_gam(full_gam))
